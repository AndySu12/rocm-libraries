...
################################################################################
"""Unit tests for TDMWaveCross -- which wave issues which member of a TDM group.

TDMWaveCross is a knob on the wave *assignment*. TDMFuse picks the grouping
...
Every kernel of the b8-3 silicon evidence chain was generated at TDMWaveCross=0,
...
from Tensile.Components.TDMFuse import (
    TDM_GROUPS,
    TDM_TENSORS,
    TDM_WAVE_CROSS_CROSSED,
    TDM_WAVE_CROSS_DEFAULT,
    _crossGroups,
    liveGroups,
    tdmMemberIsLive,
    partitionedGroups,
...
def _ks(fuse=0, cross=TDM_WAVE_CROSS_DEFAULT, numWaves=4, pgrA=2, pgrB=2,
...
def test_default_is_the_identity_on_the_shipped_partition(tc):
    """THE gate. At TDMWaveCross=0 the assignment reproduces the shipped wave
...
def test_crossing_swaps_the_second_group_against_the_first(fuse, expected):
    ...
    assert tdmWaveAssignment(_ks(fuse=fuse, cross=TDM_WAVE_CROSS_CROSSED)) == expected
...
def test_crossing_is_its_own_inverse():
    ...
    for fuse in (0, 1):
        assert (tdmWaveAssignment(_ks(fuse=fuse, cross=TDM_WAVE_CROSS_CROSSED))
...
def test_crossing_moves_no_tensor_off_the_waves_altogether():
    ...
    for fuse in (0, 1):
        ...
        crossed = tdmWaveAssignment(_ks(fuse=fuse, cross=TDM_WAVE_CROSS_CROSSED))
...
def test_crossed_default_grouping_balances_every_wave():
    ...
    for members in tdmWaveAssignment(_ks(fuse=0, cross=TDM_WAVE_CROSS_CROSSED)).values():
...
def test_default_is_never_rejected():
    for ks in _everyShape():
        assert tdmWaveCrossRejectReason(ks) is None, ks


@pytest.mark.parametrize("fuse", [0, 1])
def test_two_partitioned_groups_admit_crossing(fuse):
    ks = _ks(fuse=fuse, cross=TDM_WAVE_CROSS_CROSSED)
    assert len(partitionedGroups(ks)) == 2
    assert tdmWaveCrossRejectReason(ks) is None
...
def test_amx_has_one_partitioned_group_so_nothing_to_cross():
    ...
    ks = _ks(fuse=2, cross=TDM_WAVE_CROSS_CROSSED)
    assert partitionedGroups(ks) == (("A", "MXSA", "MXSB"),)
    assert "leaves 1 group(s) split across waves" in tdmWaveCrossRejectReason(ks)
...
def test_a_scaleless_type_degenerates_and_is_rejected(mxBlockA, mxBlockB):
    ...
    ks = _ks(fuse=0, cross=TDM_WAVE_CROSS_CROSSED,
             mxBlockA=mxBlockA, mxBlockB=mxBlockB)
    assert len(partitionedGroups(ks)) < 2
    assert "nothing to cross" in tdmWaveCrossRejectReason(ks)


@pytest.mark.parametrize("numWaves", [0, 1])
def test_one_wave_has_no_waves_to_cross_between(numWaves):
    reason = tdmWaveCrossRejectReason(
        _ks(fuse=0, cross=TDM_WAVE_CROSS_CROSSED, numWaves=numWaves))
...
def test_reject_reason_names_the_grouping_not_the_tdmfuse_integer():
    ...
    reason = tdmWaveCrossRejectReason(_ks(fuse=2, cross=TDM_WAVE_CROSS_CROSSED))
...
def test_a_declined_grouping_falls_back_before_the_cross_is_judged():
    ...
    ks = _ks(fuse=1, cross=TDM_WAVE_CROSS_CROSSED, UseSubtileImpl=True)
...
# ---------------------------------------------------------------------------
def _ldsKs(fuse=0, cross=TDM_WAVE_CROSS_DEFAULT, depthU=256, mt0=64, mt1=512,
           pgrA=2, pgrB=2):
...
def test_crossing_conserves_total_lds(fuse):
    ...
    crossed = tdmWaveLdsBytes(_ldsKs(fuse=fuse, cross=TDM_WAVE_CROSS_CROSSED))
...
def test_crossing_evens_out_the_per_wave_share_at_the_default_grouping():
    ...
    def spread(ks):
        ...
        return max(v) - min(v)
    assert spread(_ldsKs(fuse=0, cross=TDM_WAVE_CROSS_CROSSED)) \
...
def test_issue_order_is_unchanged_by_crossing():
    ...
    for fuse, pgrA, pgrB in itertools.product((0, 1), (1, 2), (1, 2)):
        ...
        crossed = tdmWaveIssueOrder(
            _ks(fuse=fuse, cross=TDM_WAVE_CROSS_CROSSED, pgrA=pgrA, pgrB=pgrB), "A", "B")
...
def test_default_adds_no_token_to_the_kernel_name():
    ...
    assert "TDMWC" not in N.getKernelNameMin(
        _nameState(TDMWaveCross=TDM_WAVE_CROSS_DEFAULT), splitGSU=False)
...
def test_crossed_adds_a_token_so_it_cannot_dedup_onto_the_default():
    ...
    crossed = N.getKernelNameMin(
        _nameState(TDMWaveCross=TDM_WAVE_CROSS_CROSSED), splitGSU=False)
    assert "TDMWC1" in crossed
    assert plain != crossed
...
def test_the_default_name_is_identical_to_one_with_the_key_absent():
    ...
    absent = _nameState()
    assert "TDMWaveCross" not in absent
